...
只读 `proxies` 段。订阅是一份完整的 mihomo 配置，其余部分由机场生成——`hosts`、`rules`、`dns`
这些段落出现畸形是有先例的。整份文档解析不了时，会退一步只解析 `proxies` 段并打一条警告：
为了一个没人读的段落丢掉整份订阅并不值得。但 `proxies` 段自己坏掉仍然是致命的，因为把它当成
「机场撤掉了所有节点」会把策略组清空。
...
拉取**失败**时不会等这么久：10 秒后重试，此后每失败一次翻倍，封顶到 `interval`——但只要一个
节点都还没有，就压到 5 分钟，因为那意味着所有策略组都是空的，流量无处可去。否则路由器只要在
上行通之前开机，就会一直空着到第二天。
...
若 `http_client.headers` 里也写了 `User-Agent`，以那一份为准——它在发请求时统一贴上去，会盖掉
这里设的值。两处挑一处写即可。
