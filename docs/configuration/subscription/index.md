...
Only the `proxies` section is read. A subscription is a whole mihomo config, and the rest of it
is generated by the airport — `hosts`, `rules` and `dns` sections have been seen to arrive
malformed. If the document does not parse as a whole, the `proxies` section is read on its own
and a warning says so; a subscription is not worth discarding over a section nobody reads. A
break inside `proxies` itself is still fatal, since treating it as "the airport withdrew every
node" would empty the groups.
